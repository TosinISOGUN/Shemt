...
export function AnalyticsPage() {
  ...
  return (
    ...
    >
      {error && (
        <div className="bg-destructive/10 border border-destructive/20 text-destructive px-4 py-3 rounded-lg flex items-center justify-between shadow-sm animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="flex items-center gap-3">
            <Zap className="h-5 w-5 rotate-180" />
            <span className="text-sm font-bold">{error}</span>
          </div>
          <Button variant="ghost" size="sm" onClick={() => setError(null)} className="h-8 hover:bg-destructive/10">Dismiss</Button>
        </div>
      )}
      ...
      <div className="grid grid-cols-12 gap-8">
              ...
              <div className="flex gap-2">
                {metrics?.revenue && (
                  <div className={cn(
                    ...
                    {metrics.revenue.change >= 0 ? <TrendingUp className="h-4 w-4" /> : <TrendingDown className="h-4 w-4" />}
                    <span className="text-xs font-bold">{metrics.revenue.change > 0 ? '+' : ''}{metrics.revenue.change}% vs last period</span>
                  </div>
                )}
              </div>
            </CardHeader>
        ...
        <div className="col-span-12 lg:col-span-4 space-y-8">
                ...
                <div className="space-y-4">
                  ...
                  {insights.length > 3 && (
                    <Button
                      variant="outline"
                      size="sm"
                      className="w-full bg-transparent border-primary/10 hover:bg-primary/10 text-primary text-xs h-9"
                      onClick={() => setShowAllInsights(!showAllInsights)}
                    >
                      {showAllInsights ? 'Show Less' : 'View All AI Insights'}
                    </Button>
                  )}
                </div>
              )}
          ...
          <Card className="border-border/40 bg-card/10 overflow-hidden">
            <CardHeader className="py-4 border-b border-border/20 flex flex-row items-center justify-between">
              <CardTitle className="text-sm font-bold flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20">
                ...
                <span className="text-[9px] font-bold text-emerald-500 uppercase tracking-tight">Active</span>
              </div>
            </CardHeader>
            <CardContent className="p-0">
              {loading ? (
                ...
                <div className="divide-y divide-border/5">
                  {activity.map((item, idx) => {
                    const Icon = item.type === 'signup' ? UserPlus :
                      item.type === 'paid' ? DollarSign :
                        item.type === 'activated' ? Zap : Eye;
                    return (
                      <div key={item.id} className="p-4 flex items-start gap-3 hover:bg-muted/5 transition-colors cursor-default group">
                        <div className={cn(
                          "h-8 w-8 rounded-lg flex items-center justify-center shrink-0 transition-transform group-hover:scale-110",
                          item.type === 'paid' ? "bg-emerald-500/10 text-emerald-500" :
                            item.type === 'signup' ? "bg-blue-500/10 text-blue-500" :
                              item.type === 'activated' ? "bg-amber-500/10 text-amber-500" : "bg-muted text-muted-foreground"
                        )}>
                          <Icon className="h-4 w-4" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-bold text-foreground truncate capitalize">{item.description}</p>
                          <p className="text-[10px] text-muted-foreground font-medium">{item.time}</p>
                        </div>
                        <ChevronRight className="h-3 w-3 text-muted-foreground/30 mt-1 opacity-0 group-hover:opacity-100 transition-opacity" />
                      </div>
                    )
                  })}
                </div>
              ...
              {activity.length > 0 && (
                <div className="p-3 bg-muted/5 border-t border-border/10">
